...
object PdfExporter {
    ...
    fun export(
        ...
        val titlePaint = Paint().apply { color = Color.rgb(241, 245, 249); textSize = 20f; isFakeBoldText = true }
        val subtitlePaint = Paint().apply { color = Color.rgb(148, 163, 184); textSize = 12f }
        val gridPaint = Paint().apply { color = Color.rgb(51, 65, 85); strokeWidth = 0.5f }
        val medianBedPaint = Paint().apply { color = Color.rgb(99, 102, 241); strokeWidth = 1.5f; pathEffect = DashPathEffect(floatArrayOf(6f, 6f), 0f) }
        val medianWakePaint = Paint().apply { color = Color.rgb(20, 184, 166); strokeWidth = 1.5f; pathEffect = DashPathEffect(floatArrayOf(6f, 6f), 0f) }
        val latencyPaint = Paint().apply { color = Color.rgb(75, 85, 99) }
        val wakePaint = Paint().apply { color = Color.rgb(251, 146, 60) }
        val labelPaint = Paint().apply { color = Color.rgb(100, 116, 139); textSize = 9f }
        ...
        canvas.drawText(
            "Ø Dauer: ${DateTimeUtil.formatDuration(avgDuration.toInt())}  |  " +
                "Ø Qualität: ${String.format(Locale.US, "%.1f", avgQuality)}/10  |  " +
                "Ø Unterbrechungen: ${String.format(Locale.US, "%.1f", avgInterruptions)}  |  " +
        ...
        val timeLabels = listOf("18:00", "20:00", "22:00", "00:00", "02:00", "04:00", "06:00", "08:00", "10:00", "12:00", "14:00", "16:00", "18:00")
        ...
        canvas.drawLine(medWakeX, timelineTop, medWakeX, timelineTop + timelineHeight, medianWakePaint)
        ...
        sorted.forEachIndexed { i, entry ->
            ...
            val sleepPaint = Paint().apply { color = qualityColorInt(entry.quality) }
    ...
    private fun qualityColorInt(quality: Int): Int = when {
        quality >= 8 -> Color.rgb(16, 185, 129)
        quality >= 5 -> Color.rgb(99, 102, 241)
        else -> Color.rgb(244, 63, 94)
    }
}
